<!-- page for deleting personal notes -->
<?php
session_start(); //#1
...
// Check for a valid notes ID, through GET or POST
...
if ( (isset($_POST['id'])) && (is_numeric($_POST['id'])) ) {
 ...
 mysqli_stmt_prepare($q, 'DELETE FROM personal_notes WHERE notes_id=? LIMIT 1');
 ...
 // Print a message:
 echo '<h3 class="text-center">The note has been deleted.</h3>';
 } else { // If the query did not run OK display public message
 echo '<p class="text-center">The Note could not be deleted.';
 echo '<br>Either the note does not exist or due to a system error.</p>';
 
 }
 } else { // User did not confirm deletion.
 echo '<h3 class="text-center">The note has NOT been deleted as ';
 echo 'you requested</h3>';
 }
 } else { // Show the form. #3
 $q = mysqli_stmt_init($dbcon);
 $query = "SELECT title FROM personal_notes WHERE notes_id=?";
 //$query .= "cows WHERE cow_id=?";
